...
from main.consts import X_FLDS, X_FLDS_IDS, X_CARD_HD, X_CARD_SR1, X_CARD_QT, X_CARD_BT1, X_CARD_HT, X_CARD_SRN, X_CARD_BTN, \
...
def generate_card_template(answer_id: int):
    ...
    """
    asw: str = '{{' + X_FLDS[X_FLDS_IDS[answer_id + 1]] + '}}'
    if answer_id == 1:
        card_front: str = X_CARD_HD + X_CARD_SR1 + X_CARD_QT + X_CARD_BT1 % '...'
        card_back: str = X_CARD_HD + X_CARD_SR1 + X_CARD_QT + X_CARD_BT1 % asw
    else:
        hint: str = '<ul>'
...
def add_x_model(col: Collection) -> Dict[str, List[str]]:
    ...
    x_model: Dict = models.new(X_MODEL_NAME)
    # Add fields:
    for fldId in X_FLDS_IDS:
        fld: Dict = models.newField(X_FLDS[fldId])
...
def set_x_model_fields(x_model: Dict):
    """
