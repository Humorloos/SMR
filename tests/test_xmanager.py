...
from smr.xmanager import is_empty_node, get_topic_content, get_non_empty_sibling_nodes, get_parent_node, \
    get_topic_image, get_node_hyperlink, XManager, NodeNotFoundError
...
def test_get_node_title(tag_for_tests):
    # when
    title = get_topic_content(tag_for_tests)
    # then
...
def test_get_tag_by_id(x_manager):
    # when
    tag = x_manager.get_node_by_id(cts.ONE_OR_MORE_AMINE_GROUPS_NODE_ID)
    # then
...
def test_get_node_content(x_manager, tag_for_tests):
    # when
    node_content = x_manager.get_topic_content(tag=tag_for_tests)
    # then
...
def test_get_node_content_with_image(x_manager):
    # given
    tag = x_manager.get_node_by_id(cts.NEUROTRANSMITTERS_NODE_ID)
    # when
    node_content = x_manager.get_topic_content(tag=tag)
    # then
...
def test_get_node_content_with_media(x_manager):
    # given
    tag = x_manager.get_node_by_id('23nu73chqkkkem455dit5p8stu')
    # when
    node_content = x_manager.get_topic_content(tag=tag)
    # then
...
def test_get_node_content_with_media_via_hyperlink(x_manager):
    # given
    tag = x_manager.get_node_by_id(cts.SEROTONIN_MEDIA_HYPERLINK_NODE_ID)
    # when
    node_content = x_manager.get_topic_content(tag=tag)
    # then
...
def test_get_parent_node(x_manager):
    ...
    # when
    parent_node = get_parent_node(x_manager.get_node_by_id(cts.EMPTY_NODE_TAG_ID))
    # then
...
def test_get_non_empty_sibling_nodes(x_manager):
    ...
    # when
    sibling_nodes = get_non_empty_sibling_nodes(x_manager.get_node_by_id(cts.EMPTY_NODE_TAG_ID))
    # then
...
def test_get_node_image(x_manager):
    ...
    tag = x_manager.get_node_by_id(cts.NEUROTRANSMITTERS_NODE_ID)
    # when
    node_image = get_topic_image(tag)
    # then
...
def test_get_node_hyperlink(x_manager):
    # given
    tag = x_manager.get_node_by_id('1s7h0rvsclrnvs8qq9u71acml5')
    # when
...
def test_set_node_title(x_manager, tag_for_tests):
    ...
    # then
    assert get_topic_content(tag_for_tests) == title
...
def test_set_node_image_remove(x_manager, changed_collection_with_example_map, smr_world_with_example_map):
    ...
    # then
    assert get_topic_image(tag) is None
...
def test_set_node_image_add(x_manager, changed_collection_with_example_map, smr_world_with_example_map, tag_for_tests):
    ...
    x_manager.set_node_image(tag=tag, note_image=cts.NEW_IMAGE_NAME, node_image=None,
                             ...
                             smr_world=smr_world_with_example_map)
    # then
    assert get_topic_image(tag) == expected_image
