...
class Graph(BaseMainGraph):
    ...
    def __init__(self, filename, clone=None, exclusive=True, sqlite_tmp_dir="",
                 world=None, profiling=False):
        exists = os.path.exists(filename) and os.path.getsize(
            filename)  # BEFORE creating db!
        initialize_db = (clone is None) and (
                (filename == ":memory:") or (not exists))

        if clone and (filename != ":memory:"):
            if exists: raise ValueError(
                "Cannot save existent quadstore in '%s': File already exists! Use a new filename for saving quadstore or, for opening an already existent quadstore, do not create any triple before calling set_backend()." % filename)

        if sqlite_tmp_dir: os.environ["SQLITE_TMPDIR"] = sqlite_tmp_dir

        if exclusive:
            self.db = sqlite3.connect(filename, isolation_level="EXCLUSIVE",
        ...
        if sqlite_tmp_dir:
            try:
                self.db.execute(
                    """PRAGMA temp_store_directory = '%s'""" % sqlite_tmp_dir)
            except:
                pass  # Deprecated PRAGMA
        ...
        if initialize_db:
            ...
            self.execute(
                """CREATE TABLE store (version INTEGER, current_blank INTEGER, current_resource INTEGER)""")
            self.execute("""INSERT INTO store VALUES (8, 0, 300)""")
            self.execute(
                """CREATE TABLE objs (c INTEGER, s INTEGER, p INTEGER, o INTEGER)""")
            self.execute(
                """CREATE TABLE datas (c INTEGER, s INTEGER, p INTEGER, o BLOB, d INTEGER)""")
            ...
            self.execute(
                """CREATE TABLE ontologies (c INTEGER PRIMARY KEY, iri TEXT, last_update DOUBLE)""")
            self.execute(
                """CREATE TABLE ontology_alias (iri TEXT, alias TEXT)""")
            self.execute("""CREATE TABLE prop_fts (storid INTEGER)""")
            try:
                self.execute(
                    """CREATE TABLE resources (storid INTEGER PRIMARY KEY, iri TEXT) WITHOUT ROWID""")
            except sqlite3.OperationalError:  # Old SQLite3 does not support WITHOUT ROWID -- here it is just an optimization
                self.execute(
                    """CREATE TABLE resources (storid INTEGER PRIMARY KEY, iri TEXT)""")
            self.db.executemany("INSERT INTO resources VALUES (?,?)",
                                _universal_abbrev_2_iri.items())
            self.execute(
            ...
            self.execute(
                """CREATE UNIQUE INDEX index_objs_op ON objs(o,p,c,s)""")  # c is for onto.classes(), etc

            self.execute("""CREATE INDEX index_datas_sp ON datas(s,p)""")
            # self.execute("""CREATE INDEX index_datas_op ON datas(o,p)""")
            self.execute(
                """CREATE UNIQUE INDEX index_datas_op ON datas(o,p,c,d,s)""")
            self.indexed = True
            ...
            version, self.current_blank, self.current_resource = self.execute(
                "SELECT version, current_blank, current_resource FROM store").fetchone()
            self.current_resource = \
                self.execute("SELECT MAX(storid) FROM resources").fetchone()[0]
            ...
            if version == 1:
                print(
                    "* Owlready2 * Converting quadstore to internal format 2...",
                    file=sys.stderr)
                self.execute(
                    """CREATE TABLE ontology_alias (iri TEXT, alias TEXT)""")
            ...
            if version == 2:
                print(
                    "* Owlready2 * Converting quadstore to internal format 3...",
                    file=sys.stderr)
                self.execute(
                    """CREATE TABLE prop_fts (fts INTEGER PRIMARY KEY, storid TEXT)""")
                self.execute("""UPDATE store SET version=3""")
                self.db.commit()

            if version == 3:
                print(
                    "* Owlready2 * Converting quadstore to internal format 4 (this can take a while)...",
                    file=sys.stderr)
                self.execute(
                    """CREATE TABLE objs (c INTEGER, s TEXT, p TEXT, o TEXT)""")
                self.execute(
                    """CREATE TABLE datas (c INTEGER, s TEXT, p TEXT, o BLOB, d TEXT)""")
                ...
                self.db.executemany("INSERT INTO datas VALUES (?,?,?,?,?)",
                                    datas)
            ...
            if version == 4:
                print(
                    "* Owlready2 * Converting quadstore to internal format 5 (this can take a while)...",
                    file=sys.stderr)
                self.execute(
                    """CREATE TABLE objs2 (c INTEGER, s INTEGER, p INTEGER, o INTEGER)""")
                self.execute(
                    """CREATE TABLE datas2 (c INTEGER, s INTEGER, p INTEGER, o BLOB, d INTEGER)""")

                _BASE_62 = {c: i for (i, c) in enumerate(
                    "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")}

                def _base_62_2_int(storid):
                    if storid.startswith("_"):
                        sgn = -1;
                        storid = storid[1:]
                    else:
                        sgn = 1
                ...
                try:
                    self.execute(
                        """CREATE TABLE resources2 (storid INTEGER PRIMARY KEY, iri TEXT) WITHOUT ROWID""")
                except sqlite3.OperationalError:  # Old SQLite3 does not support WITHOUT ROWID -- here it is just an optimization
                    self.execute(
                        """CREATE TABLE resources2 (storid INTEGER PRIMARY KEY, iri TEXT)""")
                l = []
                for storid, iri in self.execute(
                        """SELECT storid, iri FROM resources"""):
                ...
                prop_fts = {storid: fts for (fts, storid) in self.execute(
                    """SELECT fts, storid FROM prop_fts;""")}
                prop_fts2 = {_base_62_2_int(storid): fts for (storid, fts) in
                             prop_fts.items()}
                for fts in prop_fts.values():
                    self.execute("""DROP TABLE fts_%s""" % fts)
                    self.execute(
                        """DROP TRIGGER IF EXISTS fts_%s_after_insert""" % fts)
                    self.execute(
                        """DROP TRIGGER IF EXISTS fts_%s_after_delete""" % fts)
                    self.execute(
                        """DROP TRIGGER IF EXISTS fts_%s_after_update""" % fts)
            ...
            if version == 5:
                print(
                    "* Owlready2 * Converting quadstore to internal format 6 (this can take a while)...",
                    file=sys.stderr)
                self.execute("""DROP INDEX IF EXISTS index_objs_po""")
                self.execute("""DROP INDEX IF EXISTS index_datas_po""")
            ...
            if version == 6:
                print(
                    "* Owlready2 * Converting quadstore to internal format 7 (this can take a while)...",
                    file=sys.stderr)

                prop_fts2 = {storid for (storid,) in
                             self.execute("""SELECT storid FROM prop_fts;""")}
                for prop_storid in prop_fts2:
                    self.execute("""DELETE FROM prop_fts WHERE storid = ?""",
                                 (prop_storid,))
                    self.execute("""DROP TABLE fts_%s""" % prop_storid)
                    self.execute(
                        """DROP TRIGGER fts_%s_after_insert""" % prop_storid)
                    self.execute(
                        """DROP TRIGGER fts_%s_after_delete""" % prop_storid)
                    self.execute(
                        """DROP TRIGGER fts_%s_after_update""" % prop_storid)
                self.prop_fts = set()
                for prop_storid in prop_fts2: self.enable_full_text_search(
                    prop_storid)
    ...
    def select_abbreviate_method(self):
        ...
        if nb < 100000:
            iri_storid = self.execute(
                "SELECT iri, storid FROM resources").fetchall()
            self._abbreviate_d = dict(iri_storid)
            self._unabbreviate_d = dict(
                (storid, iri) for (iri, storid) in iri_storid)
    ...
    def fix_base_iri(self, base_iri, c=None):
        ...
        use_slash = self.execute(
            "SELECT iri FROM resources WHERE iri=? LIMIT 1",
            ("%s/" % base_iri,)).fetchone()
        if use_slash: return "%s/" % base_iri
        use_hash = self.execute(
            "SELECT resources.iri FROM resources WHERE SUBSTR(resources.iri, 1, ?)=? LIMIT 1",
            (len(base_iri) + 1, "%s#" % base_iri,)).fetchone()
    ...
    def sub_graph(self, onto):
        ...
        c = self.execute("SELECT c FROM ontologies WHERE iri=?",
                         (onto.base_iri,)).fetchone()
        if c is None:
            c = self.execute(
                "SELECT ontologies.c FROM ontologies, ontology_alias WHERE ontology_alias.alias=? AND ontologies.iri=ontology_alias.iri",
                (onto.base_iri,)).fetchone()
            if c is None:
                new_in_quadstore = True
                self.execute("INSERT INTO ontologies VALUES (NULL, ?, 0)",
                             (onto.base_iri,))
                c = self.execute("SELECT c FROM ontologies WHERE iri=?",
                                 (onto.base_iri,)).fetchone()
        c = c[0]
    ...
    def ontologies_iris(self):
        for (iri,) in self.execute(
                "SELECT iri FROM ontologies").fetchall(): yield iri

    def _abbreviate_sql(self, iri, create_if_missing=True):
        r = self.execute("SELECT storid FROM resources WHERE iri=? LIMIT 1",
                         (iri,)).fetchone()
        if r: return r[0]
    ...
    def _unabbreviate_sql(self, storid):
        return self.execute("SELECT iri FROM resources WHERE storid=? LIMIT 1",
                            (storid,)).fetchone()[0]
    ...
    def get_storid_dict(self):
        return dict(
            self.execute("SELECT storid, iri FROM resources").fetchall())

    def _new_numbered_iri(self, prefix):
        if prefix in self.last_numbered_iri:
            i = self.last_numbered_iri[prefix] = self.last_numbered_iri[
                                                     prefix] + 1
            return "%s%s" % (prefix, i)
        else:
            cur = self.execute(
                "SELECT iri FROM resources WHERE iri GLOB ? ORDER BY LENGTH(iri) DESC, iri DESC",
                ("%s*" % prefix,))
            while True:
                iri = cur.fetchone()
                if not iri: break
    ...
    def _refactor_sql(self, storid, new_iri):
        self.execute("UPDATE resources SET iri=? WHERE storid=?",
                     (new_iri, storid,))
    ...
    def commit(self):
        if self.current_changes != self.db.total_changes:
            self.current_changes = self.db.total_changes
            self.execute("UPDATE store SET current_blank=?, current_resource=?",
                         (self.current_blank, self.current_resource))
            self.db.commit()

    def context_2_user_context(self, c):
        user_c = self.c_2_onto.get(c)
        if user_c is None:
            iri = self.execute("SELECT iri FROM ontologies WHERE c=? LIMIT 1",
                               (c,)).fetchone()[0]
    ...
    def _get_obj_triples_spo_spo(self, s, p, o):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o FROM objs")
                else:
                    cur = self.execute("SELECT s,p,o FROM objs WHERE o=?", (o,))
            else:
                if o is None:
                    cur = self.execute("SELECT s,p,o FROM objs WHERE p=?", (p,))
                else:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE p=? AND o=?", (p, o,))
        else:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o FROM objs WHERE s=?", (s,))
                else:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE s=? AND o=?", (s, o,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE s=? AND p=?", (s, p,))
                else:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs INDEXED BY index_objs_sp WHERE s=? AND p=? AND o=?",
                        (s, p, o,))
        return cur.fetchall()
    ...
    def _get_data_triples_spod_spod(self, s, p, o, d):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM datas")
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE o=?", (o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE o=? AND d=?",
                            (o, d,))
            else:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM datas WHERE p=?",
                                       (p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE p=? AND o=?",
                            (p, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE p=? AND o=? AND d=?",
                            (p, o, d,))
        else:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM datas WHERE s=?",
                                       (s,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas INDEXED BY index_datas_sp WHERE s=? AND o=?",
                            (s, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas INDEXED BY index_datas_sp WHERE s=? AND o=? AND d=?",
                            (s, o, d,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM datas WHERE s=? AND p=?", (s, p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas INDEXED BY index_datas_sp WHERE s=? AND p=? AND o=?",
                            (s, p, o))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas INDEXED BY index_datas_sp WHERE s=? AND p=? AND o=? AND d=?",
                            (s, p, o, d,))
        return cur.fetchall()
    ...
    def _get_triples_spod_spod(self, s, p, o, d):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM quads")
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE o=?", (o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE o=? AND d=?",
                            (o, d,))
            else:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM quads WHERE p=?",
                                       (p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE p=? AND o=?",
                            (p, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE p=? AND o=? AND d=?",
                            (p, o, d,))
        else:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM quads WHERE s=?",
                                       (s,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE s=? AND o=?",
                            (s, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE s=? AND o=? AND d=?",
                            (s, o, d,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM quads WHERE s=? AND p=?", (s, p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE s=? AND p=? AND o=?",
                            (s, p, o))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE s=? AND p=? AND o=? AND d=?",
                            (s, p, o, d,))
        return cur.fetchall()
    ...
    def _get_obj_triples_cspo_cspo(self, c, s, p, o):
        if c is None:
            if s is None:
                if p is None:
                    if o is None:
                        cur = self.execute("SELECT c,s,p,o FROM objs")
                    else:
                        cur = self.execute("SELECT c,s,p,o FROM objs WHERE o=?",
                                           (o,))
                else:
                    if o is None:
                        cur = self.execute("SELECT c,s,p,o FROM objs WHERE p=?",
                                           (p,))
                    else:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE p=? AND o=?",
                            (p, o,))
            else:
                if p is None:
                    if o is None:
                        cur = self.execute("SELECT c,s,p,o FROM objs WHERE s=?",
                                           (s,))
                    else:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE INDEXED BY index_objs_sp s=? AND o=?",
                            (s, o,))
                else:
                    if o is None:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE s=? AND p=?",
                            (s, p,))
                    else:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE INDEXED BY index_objs_sp s=? AND p=? AND o=?",
                            (s, p, o,))
        else:
            if s is None:
                if p is None:
                    if o is None:
                        cur = self.execute("SELECT c,s,p,o FROM objs WHERE c=?",
                                           (c,))
                    else:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE c=? AND o=?",
                            (c, o,))
                else:
                    if o is None:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE c=? AND p=?",
                            (c, p,))
                    else:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE c=? AND p=? AND o=?",
                            (c, p, o,))
            else:
                if p is None:
                    if o is None:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs WHERE c=? AND s=?",
                            (c, s,))
                    else:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND o=?",
                            (c, s, o,))
                else:
                    if o is None:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND p=?",
                            (c, s, p,))
                    else:
                        cur = self.execute(
                            "SELECT c,s,p,o FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND p=? AND o=?",
                            (c, s, p, o,))
        return cur.fetchall()

    def _get_obj_triples_sp_co(self, s, p):
        return self.execute("SELECT c,o FROM objs WHERE s=? AND p=?",
                            (s, p)).fetchall()

    def _get_triples_s_p(self, s):
        for (x,) in self.execute("SELECT DISTINCT p FROM quads WHERE s=?",
                                 (s,)).fetchall(): yield x

    def _get_obj_triples_o_p(self, o):
        for (x,) in self.execute("SELECT DISTINCT p FROM quads WHERE o=?",
                                 (o,)).fetchall(): yield x
    ...
    def _get_obj_triples_sp_o(self, s, p):
        for (x,) in self.execute("SELECT o FROM objs WHERE s=? AND p=?",
                                 (s, p)).fetchall(): yield x

    def _get_data_triples_sp_od(self, s, p):
        return self.execute("SELECT o,d FROM datas WHERE s=? AND p=?",
                            (s, p)).fetchall()

    def _get_triples_sp_od(self, s, p):
        return self.execute("SELECT o,d FROM quads WHERE s=? AND p=?",
                            (s, p)).fetchall()

    def _get_data_triples_s_pod(self, s):
        return self.execute("SELECT p,o,d FROM datas WHERE s=?",
                            (s,)).fetchall()

    def _get_triples_s_pod(self, s):
        return self.execute("SELECT p,o,d FROM quads WHERE s=?",
                            (s,)).fetchall()

    def _get_obj_triples_po_s(self, p, o):
        for (x,) in self.execute("SELECT s FROM objs WHERE p=? AND o=?",
                                 (p, o)).fetchall(): yield x

    def _get_obj_triples_spi_o(self, s, p, i):
        for (x,) in self.execute(
                "SELECT o FROM objs WHERE s=? AND p=? UNION SELECT s FROM objs WHERE p=? AND o=?",
                (s, p, i, s)).fetchall(): yield x

    def _get_obj_triples_pio_s(self, p, i, o):
        for (x,) in self.execute(
                "SELECT s FROM objs WHERE p=? AND o=? UNION SELECT o FROM objs WHERE s=? AND p=?",
                (p, o, o, i)).fetchall(): yield x

    def _get_obj_triple_sp_o(self, s, p):
        r = self.execute("SELECT o FROM objs WHERE s=? AND p=? LIMIT 1",
                         (s, p)).fetchone()
        if r: return r[0]
        return None

    def _get_triple_sp_od(self, s, p):
        return self.execute("SELECT o,d FROM quads WHERE s=? AND p=? LIMIT 1",
                            (s, p)).fetchone()

    def _get_data_triple_sp_od(self, s, p):
        return self.execute("SELECT o,d FROM datas WHERE s=? AND p=? LIMIT 1",
                            (s, p)).fetchone()

    def _get_obj_triple_po_s(self, p, o):
        r = self.execute("SELECT s FROM objs WHERE p=? AND o=? LIMIT 1",
                         (p, o)).fetchone()
        if r: return r[0]
        return None
    ...
    def _has_obj_triple_spo(self, s=None, p=None, o=None):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s FROM objs LIMIT 1")
                else:
                    cur = self.execute("SELECT s FROM objs WHERE o=? LIMIT 1",
                                       (o,))
            else:
                if o is None:
                    cur = self.execute("SELECT s FROM objs WHERE p=? LIMIT 1",
                                       (p,))
                else:
                    cur = self.execute(
                        "SELECT s FROM objs WHERE p=? AND o=? LIMIT 1", (p, o))
        else:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s FROM objs WHERE s=? LIMIT 1",
                                       (s,))
                else:
                    cur = self.execute(
                        "SELECT s FROM objs INDEXED BY index_objs_sp WHERE s=? AND o=? LIMIT 1",
                        (s, o))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM objs WHERE s=? AND p=? LIMIT 1", (s, p))
                else:
                    cur = self.execute(
                        "SELECT s FROM objs INDEXED BY index_objs_sp WHERE s=? AND p=? AND o=? LIMIT 1",
                        (s, p, o))
    ...
    def _has_data_triple_spod(self, s=None, p=None, o=None, d=None):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s FROM datas LIMIT 1")
                elif d is None:
                    cur = self.execute("SELECT s FROM datas WHERE o=? LIMIT 1",
                                       (o,))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE o=? AND d=? LIMIT 1",
                        (o, d,))
            else:
                if o is None:
                    cur = self.execute("SELECT s FROM datas WHERE p=? LIMIT 1",
                                       (p,))
                elif d is None:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE p=? AND o=? LIMIT 1", (p, o))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE p=? AND o=? AND d=? LIMIT 1",
                        (p, o, d))
        else:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s FROM datas WHERE s=? LIMIT 1",
                                       (s,))
                elif d is None:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE s=? AND o=? LIMIT 1",
                        (s, o))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE s=? AND o=? AND d=? LIMIT 1",
                        (s, o, d))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE s=? AND p=? LIMIT 1", (s, p))
                elif d is None:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE s=? AND p=? AND o=? LIMIT 1",
                        (s, p, o))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE s=? AND p=? AND o=? AND d=? LIMIT 1",
                        (s, p, o, d))
        return not cur.fetchone() is None
    ...
    def _del_obj_triple_raw_spo(self, s, p, o):
        if s is None:
            if p is None:
                if o is None:
                    self.execute("DELETE FROM objs")
                else:
                    self.execute("DELETE FROM objs WHERE o=?", (o,))
            else:
                if o is None:
                    self.execute("DELETE FROM objs WHERE p=?", (p,))
                else:
                    self.execute("DELETE FROM objs WHERE p=? AND o=?", (p, o,))
        else:
            if p is None:
                if o is None:
                    self.execute("DELETE FROM objs WHERE s=?", (s,))
                else:
                    self.execute(
                        "DELETE FROM objs INDEXED BY index_objs_sp WHERE s=? AND o=?",
                        (s, o,))
            else:
                if o is None:
                    ...
                    self.execute(
                        "DELETE FROM objs INDEXED BY index_objs_sp WHERE s=? AND p=? AND o=?",
                        (s, p, o,))
    ...
    def _del_data_triple_raw_spod(self, s, p, o, d):
        if s is None:
            if p is None:
                if o is None:
                    self.execute("DELETE FROM datas")
                elif d is None:
                    self.execute("DELETE FROM datas WHERE o=?", (o,))
                else:
                    self.execute("DELETE FROM datas WHERE o=? AND d=?", (o, d,))
            else:
                if o is None:
                    self.execute("DELETE FROM datas WHERE p=?", (p,))
                elif d is None:
                    self.execute("DELETE FROM datas WHERE p=? AND o=?", (p, o,))
                else:
                    self.execute("DELETE FROM datas WHERE p=? AND o=? AND d=?",
                                 (p, o, d,))
        else:
            if p is None:
                if o is None:
                    self.execute("DELETE FROM datas WHERE s=?", (s,))
                elif d is None:
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE s=? AND o=?",
                        (s, o,))
                else:
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE s=? AND o=? AND d=?",
                        (s, o, d,))
            else:
                if o is None:
                    ...
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE s=? AND p=? AND o=?",
                        (s, p, o,))
                else:
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE s=? AND p=? AND o=? AND d=?",
                        (s, p, o, d,))

    def _punned_entities(self):
        from owlready2.base import rdf_type, owl_class, owl_named_individual
        cur = self.execute(
            "SELECT q1.s FROM objs q1, objs q2 WHERE q1.s=q2.s AND q1.p=? AND q2.p=? AND q1.o=? AND q2.o=?",
            (rdf_type, rdf_type, owl_class, owl_named_individual))
        return [storid for (storid,) in cur.fetchall()]

    def __bool__(self):
        return True  # Reimplemented to avoid calling __len__ in this case
...
SELECT x FROM transit""", (p, o, p)).fetchall(): yield x
    ...
    def _destroy_collect_storids(self, destroyed_storids, modified_relations,
                                 storid):
    ...
    %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) AND s < 0""" % (
            ...
            if not blank_using in destroyed_storids:
                ...
                self._destroy_collect_storids(destroyed_storids,
                                              modified_relations, blank_using)

        for (c, blank_using) in list(self.execute(
                """SELECT c, s FROM objs WHERE o=? AND p=%s AND s < 0""" % (
                        rdf_first,
                ), (storid,))):
            list_user, root, previouss, nexts, length = self._rdf_list_analyze(
                blank_using)
    ...
    def _rdf_list_analyze(self, blank):
        ...
        list_user = self.execute("SELECT s FROM objs WHERE o=? LIMIT 1",
                                 (root,)).fetchone()
    ...
    def destroy_entity(self, storid, destroyer, relation_updater,
        ...
        self._destroy_collect_storids(destroyed_storids, modified_relations,
                                      storid)

        for s, p in self.execute(
                "SELECT DISTINCT s,p FROM objs WHERE o IN (%s)" % ",".join(
                    ["?" for i in destroyed_storids]),
        ...
        for storid in destroyed_storids:
            if undoer_objs is not None:
                undoer_objs.extend(
                    self.execute("SELECT c,s,p,o FROM objs WHERE s=? OR o=?",
                                 (storid, storid)))
                undoer_datas.extend(
                    self.execute("SELECT c,s,p,o,d FROM datas WHERE s=?",
                                 (storid,)))
    ...
    def _iter_ontology_iri(self, c=None):
        if c:
            return self.execute("SELECT iri FROM ontologies WHERE c=?",
                                (c,)).fetchone()[0]
        else:
    ...
    def enable_full_text_search(self, prop_storid):
        ...
        self.execute(
            """CREATE VIRTUAL TABLE fts_%s USING fts5(s UNINDEXED, o, d UNINDEXED, content=datas, content_rowid=rowid)""" % prop_storid)
        self.execute(
            """INSERT INTO fts_%s(rowid, s, o, d) SELECT rowid, s, o, d FROM datas WHERE p=%s""" % (
                prop_storid, prop_storid))
...
CREATE TRIGGER fts_%s_after_update AFTER UPDATE ON datas WHEN new.p=%s BEGIN
  INSERT INTO fts_%s(fts_%s, rowid, s, o, d) VALUES('delete', old.rowid, old.s, old.o, old.d);
  INSERT INTO fts_%s(rowid, s, o, d) VALUES (new.rowid, new.s, new.o, new.d);
END;""" % (prop_storid, prop_storid, prop_storid, prop_storid, prop_storid,
           prop_storid, prop_storid, prop_storid, prop_storid, prop_storid,
           prop_storid, prop_storid))
...
class SubGraph(BaseSubGraph):
    ...
    def create_parse_func(self, filename=None, delete_existing_triples=True,
        ...
        if self.parent._abbreviate_d is None:
            ...
            def _abbreviate(iri):
                ...
                r = cur.execute(
                    "SELECT storid FROM resources WHERE iri=? LIMIT 1",
                    (iri,)).fetchone()
                if r:
                    abbrevs[iri] = r[0]
                    return r[0]
        ...
        def insert_objs():
            ...
            if owlready2.namespace._LOG_LEVEL: print(
                "* OwlReady2 * Importing %s object triples from ontology %s ..." % (
                    len(objs), self.onto.base_iri), file=sys.stderr)
            cur.executemany("INSERT INTO resources VALUES (?,?)", new_abbrevs)
            cur.executemany(
                "INSERT OR IGNORE INTO objs VALUES (%s,?,?,?)" % self.c, objs)
            objs.clear()
            new_abbrevs.clear()
        ...
        def insert_datas():
            ...
            if owlready2.namespace._LOG_LEVEL: print(
                "* OwlReady2 * Importing %s data triples from ontology %s ..." % (
                    len(datas), self.onto.base_iri), file=sys.stderr)
            cur.executemany(
                "INSERT OR IGNORE INTO datas VALUES (%s,?,?,?,?)" % self.c,
                datas)
            datas.clear()
        ...
        def on_finish():
            ...
            t0 = time.time()
            onto_base_iri = cur.execute(
                "SELECT resources.iri FROM objs, resources WHERE objs.c=? AND objs.o=? AND resources.storid=objs.s LIMIT 1",
                (self.c, owl_ontology)).fetchone()
            if onto_base_iri:
            ...
            if onto_base_iri.endswith("/"):
                cur.execute(
                    "UPDATE ontologies SET last_update=?,iri=? WHERE c=?",
                    (date, onto_base_iri, self.c,))
            elif onto_base_iri:
                ...
                cur.execute("UPDATE ontologies SET last_update=? WHERE c=?",
                            (date, self.c,))
    ...
    def get_last_update_time(self):
        return self.execute("SELECT last_update FROM ontologies WHERE c=?",
                            (self.c,)).fetchone()[0]

    def set_last_update_time(self, t):
        self.execute("UPDATE ontologies SET last_update=? WHERE c=?",
                     (t, self.c))
    ...
    def _set_obj_triple_raw_spo(self, s, p, o):
        ...
        self.execute("DELETE FROM objs WHERE c=? AND s=? AND p=?",
                     (self.c, s, p,))
        self.execute("INSERT INTO objs VALUES (?, ?, ?, ?)", (self.c, s, p, o))

    def _add_obj_triple_raw_spo(self, s, p, o):
        if (s is None) or (p is None) or (o is None): raise ValueError
        self.execute("INSERT OR IGNORE INTO objs VALUES (?, ?, ?, ?)",
                     (self.c, s, p, o))
        ...
        if s is None:
            if p is None:
                if o is None:
                    ...
                    self.execute("DELETE FROM objs WHERE c=? AND o=?",
                                 (self.c, o,))
            else:
                if o is None:
                    self.execute("DELETE FROM objs WHERE c=? AND p=?",
                                 (self.c, p,))
                else:
                    self.execute("DELETE FROM objs WHERE c=? AND p=? AND o=?",
                                 (self.c, p, o,))
        else:
            if p is None:
                if o is None:
                    self.execute("DELETE FROM objs WHERE c=? AND s=?",
                                 (self.c, s,))
                else:
                    self.execute(
                        "DELETE FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND o=?",
                        (self.c, s, o))
            else:
                if o is None:
                    self.execute("DELETE FROM objs WHERE c=? AND s=? AND p=?",
                    ...
                    self.execute(
                        "DELETE FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND p=? AND o=?",
                        (self.c, s, p, o,))

    def _set_data_triple_raw_spod(self, s, p, o, d):
        if (s is None) or (p is None) or (o is None) or (
                d is None): raise ValueError
        self.execute("DELETE FROM datas WHERE c=? AND s=? AND p=?",
                     (self.c, s, p,))
        self.execute("INSERT INTO datas VALUES (?, ?, ?, ?, ?)",
                     (self.c, s, p, o, d))

    def _add_data_triple_raw_spod(self, s, p, o, d):
        if (s is None) or (p is None) or (o is None) or (
                d is None): raise ValueError
        self.execute("INSERT OR IGNORE INTO datas VALUES (?, ?, ?, ?, ?)",
                     (self.c, s, p, o, d))
        ...
        if s is None:
            if p is None:
                if o is None:
                    ...
                    self.execute("DELETE FROM datas WHERE c=? AND o=?",
                                 (self.c, o,))
                else:
                    self.execute("DELETE FROM datas WHERE c=? AND o=? AND d=?",
                                 (self.c, o, d,))
            else:
                if o is None:
                    self.execute("DELETE FROM datas WHERE c=? AND p=?",
                                 (self.c, p,))
                elif d is None:
                    self.execute("DELETE FROM datas WHERE c=? AND p=? AND o=?",
                                 (self.c, p, o,))
                else:
                    self.execute(
                        "DELETE FROM datas WHERE c=? AND p=? AND o=? AND d=?",
                        (self.c, p, o, d,))
        else:
            if p is None:
                if o is None:
                    self.execute("DELETE FROM datas WHERE c=? AND s=?",
                                 (self.c, s,))
                elif d is None:
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND o=?",
                        (self.c, s, o,))
                else:
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND o=? AND d=?",
                        (self.c, s, o, d,))
            else:
                if o is None:
                    self.execute("DELETE FROM datas WHERE c=? AND s=? AND p=?",
                    ...
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND p=? AND o=?",
                        (self.c, s, p, o,))
                else:
                    self.execute(
                        "DELETE FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND p=? AND o=? AND d=?",
                        (self.c, s, p, o, d,))
    ...
    def _has_obj_triple_spo(self, s=None, p=None, o=None):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s FROM objs WHERE c=? LIMIT 1",
                                       (self.c,))
                else:
                    cur = self.execute(
                        "SELECT s FROM objs WHERE c=? AND o=? LIMIT 1",
                        (self.c, o))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM objs WHERE c=? AND p=? LIMIT 1",
                        (self.c, p,))
                else:
                    cur = self.execute(
                        "SELECT s FROM objs WHERE c=? AND p=? AND o=? LIMIT 1",
                        (self.c, p, o))
        else:
            if p is None:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM objs WHERE c=? AND s=? LIMIT 1",
                        (self.c, s,))
                else:
                    cur = self.execute(
                        "SELECT s FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND o=? LIMIT 1",
                        (self.c, s, o))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM objs WHERE c=? AND s=? AND p=? LIMIT 1",
                        (self.c, s, p,))
                else:
                    cur = self.execute(
                        "SELECT s FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND p=? AND o=? LIMIT 1",
                        (self.c, s, p, o))
        return not cur.fetchone() is None
    ...
    def _has_data_triple_spod(self, s=None, p=None, o=None, d=None):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s FROM datas WHERE c=? LIMIT 1",
                                       (self.c,))
                elif d is None:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE c=? AND o=? LIMIT 1",
                        (self.c, o))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE c=? AND o=? AND d=? LIMIT 1",
                        (self.c, o, d))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE c=? AND p=? LIMIT 1",
                        (self.c, p,))
                elif d is None:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE c=? AND p=? AND o=? LIMIT 1",
                        (self.c, p, o))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE c=? AND p=? AND o=? AND d=? LIMIT 1",
                        (self.c, p, o, d))
        else:
            if p is None:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE c=? AND s=? LIMIT 1",
                        (self.c, s,))
                elif d is None:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND o=? LIMIT 1",
                        (self.c, s, o))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND o=? AND d=? LIMIT 1",
                        (self.c, s, o, d))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s FROM datas WHERE c=? AND s=? AND p=? LIMIT 1",
                        (self.c, s, p,))
                elif d is None:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND p=? AND o=? LIMIT 1",
                        (self.c, s, p, o))
                else:
                    cur = self.execute(
                        "SELECT s FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND p=? AND o=? AND d=? LIMIT 1",
                        (self.c, s, p, o, d))
        return not cur.fetchone() is None
    ...
    def _get_obj_triples_spo_spo(self, s=None, p=None, o=None):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o FROM objs WHERE c=?",
                                       (self.c,))
                else:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE c=? AND o=?",
                        (self.c, o,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE c=? AND p=?",
                        (self.c, p,))
                else:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE c=? AND p=? AND o=?",
                        (self.c, p, o,))
        else:
            if p is None:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE c=? AND s=?",
                        (self.c, s,))
                else:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND o=?",
                        (self.c, s, o,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs WHERE c=? AND s=? AND p=?",
                        (self.c, s, p,))
                else:
                    cur = self.execute(
                        "SELECT s,p,o FROM objs INDEXED BY index_objs_sp WHERE c=? AND s=? AND p=? AND o=?",
                        (self.c, s, p, o,))
        return cur.fetchall()
    ...
    def _get_data_triples_spod_spod(self, s, p, o, d=""):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM datas WHERE c=?",
                                       (self.c,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE c=? AND o=?",
                            (self.c, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE c=? AND o=? AND d=?",
                            (self.c, o, d,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM datas WHERE c=? AND p=?",
                        (self.c, p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE c=? AND p=? AND o=?",
                            (self.c, p, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WHERE c=? AND p=? AND o=? AND d=?",
                            (self.c, p, o, d,))
        else:
            if p is None:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM datas WHERE c=? AND s=?",
                        (self.c, s,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND o=?",
                            (self.c, s, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas WINDEXED BY index_datas_sp HERE c=? AND s=? AND o=? AND d=?",
                            (self.c, s, o, d,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM datas WHERE c=? AND s=? AND p=?",
                        (self.c, s, p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND p=? AND o=?",
                            (self.c, s, p, o))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM datas INDEXED BY index_datas_sp WHERE c=? AND s=? AND p=? AND o=? AND d=?",
                            (self.c, s, p, o, d,))
        return cur.fetchall()
    ...
    def _get_triples_spod_spod(self, s, p, o, d=""):
        if s is None:
            if p is None:
                if o is None:
                    cur = self.execute("SELECT s,p,o,d FROM quads WHERE c=?",
                                       (self.c,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND o=?",
                            (self.c, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND o=? AND d=?",
                            (self.c, o, d,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM quads WHERE c=? AND p=?",
                        (self.c, p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND p=? AND o=?",
                            (self.c, p, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND p=? AND o=? AND d=?",
                            (self.c, p, o, d,))
        else:
            if p is None:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM quads WHERE c=? AND s=?",
                        (self.c, s,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND s=? AND o=?",
                            (self.c, s, o,))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND s=? AND o=? AND d=?",
                            (self.c, s, o, d,))
            else:
                if o is None:
                    cur = self.execute(
                        "SELECT s,p,o,d FROM quads WHERE c=? AND s=? AND p=?",
                        (self.c, s, p,))
                else:
                    if d is None:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND s=? AND p=? AND o=?",
                            (self.c, s, p, o))
                    else:
                        cur = self.execute(
                            "SELECT s,p,o,d FROM quads WHERE c=? AND s=? AND p=? AND o=? AND d=?",
                            (self.c, s, p, o, d,))
        return cur.fetchall()

    def _get_obj_triples_s_po(self, s):
        return self.execute("SELECT p,o FROM objs WHERE c=? AND s=?",
                            (self.c, s,)).fetchall()

    def _get_obj_triples_sp_o(self, s, p):
        for (x,) in self.execute("SELECT o FROM objs WHERE c=? AND s=? AND p=?",
                                 (self.c, s, p,)).fetchall(): yield x

    def _get_obj_triples_sp_co(self, s, p):
        return self.execute("SELECT c,o FROM objs WHERE c=? AND s=? AND p=?",
                            (self.c, s, p,)).fetchall()

    def _get_triples_sp_od(self, s, p):
        return self.execute("SELECT o,d FROM quads WHERE c=? AND s=? AND p=?",
                            (self.c, s, p)).fetchall()

    def _get_data_triples_sp_od(self, s, p):
        return self.execute("SELECT o,d FROM datas WHERE c=? AND s=? AND p=?",
                            (self.c, s, p)).fetchall()

    def _get_data_triples_s_pod(self, s):
        return self.execute("SELECT p,o,d FROM datas WHERE c=? AND s=?",
                            (self.c, s)).fetchall()

    def _get_triples_s_pod(self, s):
        return self.execute("SELECT p,o,d FROM quads WHERE c=? AND s=?",
                            (self.c, s)).fetchall()

    def _get_obj_triples_po_s(self, p, o):
        for (x,) in self.execute("SELECT s FROM objs WHERE c=? AND p=? AND o=?",
                                 (self.c, p, o)).fetchall(): yield x

    def _get_obj_triples_spi_o(self, s, p, i):
        for (x,) in self.execute(
                "SELECT o FROM objs WHERE c=? AND s=? AND p=? UNION SELECT s FROM objs WHERE c=? AND p=? AND o=?",
                (self.c, s, p, self.c, i, s)).fetchall(): yield x

    def _get_obj_triples_pio_s(self, p, i, o):
        for (x,) in self.execute(
                "SELECT s FROM objs WHERE c=? AND p=? AND o=? UNION SELECT o FROM objs WHERE c=? AND s=? AND p=?",
                (self.c, p, o, self.c, o, i)).fetchall(): yield x

    def _get_obj_triple_sp_o(self, s, p):
        r = self.execute("SELECT o FROM objs WHERE c=? AND s=? AND p=? LIMIT 1",
                         (self.c, s, p,)).fetchone()
        if r: return r[0]
        return None
    ...
    def _get_triple_sp_od(self, s, p):
        return self.execute(
            "SELECT o,d FROM quads WHERE c=? AND s=? AND p=? LIMIT 1",
            (self.c, s, p)).fetchone()

    def _get_data_triple_sp_od(self, s, p):
        return self.execute(
            "SELECT o,d FROM datas WHERE c=? AND s=? AND p=? LIMIT 1",
            (self.c, s, p,)).fetchone()

    def _get_obj_triple_po_s(self, p, o):
        r = self.execute("SELECT s FROM objs WHERE c=? AND p=? AND o=? LIMIT 1",
                         (self.c, p, o,)).fetchone()
        if r: return r[0]
        return None
    ...
    def _get_triples_s_p(self, s):
        for (x,) in self.execute(
                "SELECT DISTINCT p FROM quads WHERE c=? AND s=?",
                (self.c, s,)).fetchall(): yield x

    def _get_obj_triples_o_p(self, o):
        for (x,) in self.execute(
                "SELECT DISTINCT p FROM quads WHERE c=? AND o=?",
                (self.c, o,)).fetchall(): yield x

    def _get_obj_triples_cspo_cspo(self, c, s, p, o):
        return [(self.c, s, p, o) for (s, p, o) in
                self._get_obj_triples_spo_spo(s, p, o)]

    def search(self, prop_vals, c=None, debug=False):
        return self.parent.search(prop_vals, self.c, debug)

    def __len__(self):
        return self.execute("SELECT COUNT() FROM quads WHERE c=?",
                            (self.c,)).fetchone()[0]
...
class _SearchMixin(list):
    ...
    def _do_search(self):
        ...
        return (self.world._get_by_storid(o) for (o,) in
                self.world.graph.execute(sql, params).fetchall())
...
class _PopulatedSearchList(FirstList):
    __slots__ = ["world", "prop_vals", "_c", "id", "transits", "tables",
                 "conditions", "params", "alternatives", "excepts",
                 "except_conditions", "except_params", "nested_searchs",
                 "target"]
    ...
    def __init__(self, world, prop_vals, c=None, case_sensitive=True):
        ...
        for k, v, d in prop_vals:
            ...
            if n == 1:
                self.target = i
                if d == "quads":
                    self.tables.append("quads q%s" % i)
                elif d is None:
                ...
                if d == "quads":
                    ...
                    if d is None:
                        self.tables.append(
                            "objs q%s INDEXED BY index_objs_sp" % i)
                    else:
                        self.tables.append(
                            "datas q%s INDEXED BY index_datas_sp" % i)
            ...
            if k == " iri":
                if n > 1: self.conditions.append(
                    "q%s.s = q%s.s" % (i, self.target))
                self.tables.append("resources")
                self.conditions.append("resources.storid = q%s.s" % i)
                if case_sensitive:
                    if "*" in v:
                        self.conditions.append("resources.iri GLOB ?")
                    else:
                        self.conditions.append("resources.iri = ?")
                ...
                if isinstance(v, (_UnionSearchList, _PopulatedUnionSearchList)):
                    for search in v.searches: self.transits.extend(
                        search.transits)
                    self.alternatives.append(v.explode(lambda target: [
                        ["q%s.s = q%s.s" % (i, self.target),
                         "(q%s.p = %s OR q%s.p = %s)" % (
                             i, rdf_type, i, rdfs_subclassof),
                         "q%s.o = q%s.s" % (i, target)]]))
                elif isinstance(v, (_SearchMixin, _PopulatedSearchList)):
                    self.conditions.append(
                        "(q%s.p = %s OR q%s.p = %s) AND q%s.o = q%s.s" % (
                            i, rdf_type, i, rdfs_subclassof, i, v.target))
                    self.nested_searchs.append(v)
                    self.nested_searchs.extend(v.nested_searchs)
                else:
                    if isinstance(v, Or): v = "), (".join(
                        str(c.storid) for c in v.Classes)
...
""" % (transit_name, v,
       transit_name, transit_name, rdfs_subclassof, rdf_type))
                ...
                if isinstance(v, (_UnionSearchList, _PopulatedUnionSearchList)):
                    ...
                    self.conditions.append("q%s.p = %s AND q%s.o = q%s.s" % (
                        i, rdf_type, i, v.target))
                    self.nested_searchs.append(v)
                    self.nested_searchs.extend(v.nested_searchs)
                else:
                    if isinstance(v, Or): v = "), (".join(
...
""" % (transit_name, v, transit_name, transit_name, rdfs_subclassof))
                    self.tables.append(transit_name)
                    self.conditions.append("q%s.p = %s AND q%s.o = %s.x" % (
                        i, rdf_type, i, transit_name))
                ...
                if isinstance(v, (_UnionSearchList, _PopulatedUnionSearchList)):
                    ...
                    self.alternatives.append(v.explode(lambda target: [
                        ["q%s.s = q%s.s" % (i, self.target),
                         "q%s.p = %s" % (i, rdfs_subclassof),
                         "q%s.o = q%s.s" % (i, target)]]))
                elif isinstance(v, (_SearchMixin, _PopulatedSearchList)):
                    self.conditions.append("q%s.p = %s AND q%s.o = q%s.s" % (
                        i, rdfs_subclassof, i, v.target))
                    self.nested_searchs.append(v)
                    self.nested_searchs.extend(v.nested_searchs)
                else:
                    if isinstance(v, Or): v = "), (".join(
...
""" % (transit_name, v, transit_name, transit_name, rdfs_subclassof))
                ...
                if isinstance(v, (_UnionSearchList, _PopulatedUnionSearchList)):
                    ...
                    self.alternatives.append(v.explode(lambda target: [
                        ["q%s.s = q%s.s" % (i, self.target),
                         "q%s.p = %s" % (i, k[0]),
                         "q%s.o = q%s.s" % (i, target)],
                        ["q%s.o = q%s.s" % (i, self.target),
                         "q%s.p = %s" % (i, k[0]),
                         "q%s.s = q%s.s" % (i, target)]]))
                else:
                    if isinstance(v, (_SearchMixin,
                                      _PopulatedSearchList)):  # First, to avoid comparing v with "*", which would require to populate it!
                        cond1 = "q%s.s = q%s.s AND q%s.p = ? AND q%s.o = q%s.s" % (
                            i, self.target, i, i, v.target)
                        cond2 = "q%s.o = q%s.s AND q%s.p = ? AND q%s.s = q%s.s" % (
                            i, self.target, i, i, v.target)
                        params1 = [k[0]]
                        params2 = [k[1]]
                        self.nested_searchs.append(v)
                        ...
                        cond1 = "q%s.s = q%s.s AND q%s.p = ?" % (
                            i, self.target, i)
                        cond2 = "q%s.o = q%s.s AND q%s.p = ?" % (
                            i, self.target, i)
                        params1 = [k[0]]
                        params2 = [k[1]]
                    else:
                        cond1 = "q%s.s = q%s.s AND q%s.p = ? AND q%s.o = ?" % (
                            i, self.target, i, i)
                        cond2 = "q%s.o = q%s.s AND q%s.p = ? AND q%s.s = ?" % (
                            i, self.target, i, i)
                        params1 = [k[0], v]
                        params2 = [k[1], v]

                    current_table = self.tables.pop()
                    if current_table.endswith(" INDEXED BY index_objs_sp"):
                        current_table2 = "%sop" % current_table[:-2]
                    else:
                        current_table2 = current_table
                    self.alternatives.append((([current_table], cond1, params1),
                                              ([current_table2], cond2,
                                               params2)))
                ...
                if isinstance(v, FTS):
                    ...
                    if isinstance(v, (
                            _UnionSearchList, _PopulatedUnionSearchList)):
                        alternatives = []
                        for search in v.searches:
                            if search.excepts: raise NotImplementedError(
                                "Nested search with union and exception are not supported.")
                            self.transits.extend(search.transits)
                            if search.alternatives:
                                for combination in all_combinations(
                                        search.alternatives):
                                    combination_tabless, combination_conditions, combination_paramss = zip(
                                        *combination)
                                    alternatives.append((search.tables + [t for
                                                                          ts in
                                                                          combination_tabless
                                                                          for t
                                                                          in
                                                                          ts],
                                                         " AND ".join(
                                                             search.conditions + combination_conditions + [
                                                                 "q%s.o = q%s.s" % (
                                                                     i,
                                                                     search.target)]),
                                                         search.params + [p for
        ...
        if self.excepts:
            for except_p in self.excepts:  # Use only quads because it may contain several properties mixing objs and datas
                if isinstance(except_p, tuple):  # Prop with inverse
                    self.except_conditions.append(
                        "quads.s = candidates.s AND quads.p = ?")
                    self.except_conditions.append(
                        "quads.o = candidates.s AND quads.p = ?")
    ...
    def sql_components(self, last_request=True):
        transits = self.transits + [x for search in self.nested_searchs for x in
                                    search.transits]
        tables = self.tables + [x for search in self.nested_searchs for x in
                                search.tables]
        conditions = self.conditions + [x for search in self.nested_searchs for
                                        x in search.conditions]
        params = self.params + [x for search in self.nested_searchs for x in
                                search.params]

        if self.nested_searchs:
            for search in self.nested_searchs:
                if search.excepts: raise ValueError(
                    "Nested searches with exclusions are not supported!")

        if not self.alternatives:
            sql = "SELECT DISTINCT q%s.s FROM %s WHERE %s" % (
                self.target, ", ".join(tables), " AND ".join(conditions))
            ...
            for combination in all_combinations(self.alternatives):
                combination_tabless, combination_conditions, combination_paramss = zip(
                    *combination)
                sql = "SELECT DISTINCT q%s.s FROM %s WHERE %s" % (self.target,
                                                                  ", ".join(
                                                                      tables + [
                                                                          t for
                                                                          ts in
                ...
                for combination_params in combination_paramss: params.extend(
                    combination_params)
...
EXCEPT %s""" % ("\nUNION ALL ".join(
                """SELECT candidates.s FROM candidates, quads WHERE %s""" % except_condition
                for except_condition in self.except_conditions))
    ...
    def dump(self):
        sql, params = self.sql_request()
        print("search debug:")
        prop_vals = ["(nested search)" if isinstance(i[1], (
            _SearchMixin, _PopulatedSearchList)) else i for i in self.prop_vals]
...
class _UnionSearchList(FirstList, _SearchMixin, _LazyListMixin):
    ...
    def sql_components(self, last_request=True):
        ...
        if last_request:
            sql = "SELECT DISTINCT * FROM (\n%s\n)" % "\nUNION ALL\n".join(
                sql2 for (transits2, sql2, params2) in transits_sqls_params)
        else:
    ...
    def explode(self, gen):
        ...
        for search in self.searches:
            if search.excepts: raise NotImplementedError(
                "Nested search with both union and exception are not supported.")
            if search.alternatives:
                for combination in all_combinations(search.alternatives):
                    combination_tabless, combination_conditions, combination_paramss = zip(
                        *combination)
                    for alternative_conditions in gen(search.target):
                        alternatives.append((search.tables + [t for ts in
                                                              combination_tabless
                                                              for t in ts],
                                             " AND ".join(
                                                 search.conditions + combination_conditions + alternative_conditions),
                                             search.params + [p for ps in
                                                              combination_paramss
                                                              for p in ps]))

            else:
                for alternative_conditions in gen(search.target):
                    alternatives.append((search.tables, " AND ".join(
                        search.conditions + alternative_conditions),
                                         search.params))
