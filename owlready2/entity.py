...
class EntityClass(type):
  ...
  def __new__(MetaClass, name, superclasses, obj_dict):
    ...
    if Class is None:
      if not LOADING:
        for base in _is_a:
          if isinstance(base, ClassConstruct):
            base._set_ontology_copy_if_needed(namespace.ontology, _is_a)
  ...
  def _class_equivalent_to_changed(Class, old):
    ...
    for x in new - old:
      if isinstance(x, ClassConstruct):
        x._set_ontology_copy_if_needed(Class.namespace.ontology, Class._equivalent_to)
  ...
  def _class_is_a_changed(Class, old):
    ...
    for base in new - old:
      if isinstance(base, ClassConstruct):
        base._set_ontology_copy_if_needed(Class.namespace.ontology, Class.is_a)
