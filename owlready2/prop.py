...
class PropertyClass(EntityClass):
    ...
    def _add_is_a_triple(Prop, base):
        if base in _CLASS_PROPS:
            pass
        elif base in _TYPE_PROPS:
            Prop.namespace.ontology._add_obj_triple_spo(Prop.storid, rdf_type, base.storid)
        else:
            Prop.namespace.ontology._add_obj_triple_spo(Prop.storid, Prop._rdfs_is_a, base.storid)

    def _del_is_a_triple(Prop, base):
        if base in _CLASS_PROPS:
            pass
        elif base in _TYPE_PROPS:
            Prop.namespace.ontology._del_obj_triple_spo(Prop.storid, rdf_type, base.storid)
        else:
            Prop.namespace.ontology._del_obj_triple_spo(Prop.storid, Prop._rdfs_is_a, base.storid)

    def get_domain(Prop):
        if Prop._domain is None:
            Prop._domain = CallbackList((Prop.namespace.world._to_python(o, default_to_none=True) for o in
                                         Prop.namespace.world._get_obj_triples_sp_o(Prop.storid, rdf_domain)),
                                        Prop, PropertyClass._domain_changed)
        return Prop._domain

    def set_domain(Prop, value):
        Prop.domain.reinit(value)
    ...
    def _domain_changed(Prop, old):
        ...
        for x in new - old:
            if isinstance(x, ClassConstruct): x._set_ontology_copy_if_needed(Prop.namespace.ontology, Prop.domain)
    ...
    def _range_changed(Prop, old):
        ...
        for x in old - new:
            ...
            if isinstance(x, ClassConstruct): x._set_ontology(None)
        for x in new - old:
            if isinstance(x, ClassConstruct): x._set_ontology_copy_if_needed(Prop.namespace.ontology, Prop.range)
            x2 = _universal_datatype_2_abbrev.get(x) or x.storid
    ...
    def get_range_iri(Prop):
        if not "_range_iri" in Prop.__dict__:
            ...
            for o in Prop.namespace.world._get_obj_triples_sp_o(Prop.storid, rdf_range):
                if o > 0:
                    iris.append(Prop.namespace.world._unabbreviate(o))
                else:
                    iris.append("_:%s" % (-o))
    ...
    def _range_iri_changed(Prop, old):
        ...
        for x in old - new:
            if x.startswith("_"):
                ...
                x2 = Prop.namespace.world._abbreviate(x)
            Prop.namespace.ontology._del_obj_triple_spo(Prop.storid, rdf_range, x2)
        for x in new - old:
            if x.startswith("_"):
                x2 = -int(x[2:])
    ...
    def get_class_property_type(Prop):
        return Prop._class_property_type

    def set_class_property_type(Prop, value):
        Prop.class_property_type.reinit(value)
    ...
    def set_property_chain(Prop, value):
        Prop.property_chain.reinit(value)
    ...
    def _property_chain_changed(Prop, old):
        ...
        for x in new - old:
            x._set_ontology_copy_if_needed(Prop.namespace.ontology, Prop._property_chain)
    ...
    def __getattr__(Prop, attr):
        ...
        return IndividualValueList(
            (Prop.namespace.ontology._to_python(o, d) for o, d in
             Prop.namespace.world._get_triples_sp_od(Prop.storid, Annot.storid)),
    ...
    def set_python_name(Prop, python_name):
        if not LOADING: Prop.namespace.ontology._set_data_triple_spod(Prop.storid, owlready_python_name,
                                                                      *to_literal(python_name))
    ...
    def some(Prop, value):
        return Restriction(Prop, SOME, None, value)

    def only(Prop, value):
        return Restriction(Prop, ONLY, None, value)

    def value(Prop, value):
        return Restriction(Prop, VALUE, None, value)

    def has_self(Prop, value=True):
        return Restriction(Prop, HAS_SELF, None, value)

    def exactly(Prop, nb, value=None):
        return Restriction(Prop, EXACTLY, nb, value)

    def min(Prop, nb, value=None):
        return Restriction(Prop, MIN, nb, value)

    def max(Prop, nb, value=None):
        return Restriction(Prop, MAX, nb, value)

    def __lt__(prop, value):
        return prop.some(ConstrainedDatatype(type(value), max_exclusive=value))

    def __le__(prop, value):
        return prop.some(ConstrainedDatatype(type(value), max_inclusive=value))

    def __gt__(prop, value):
        return prop.some(ConstrainedDatatype(type(value), min_exclusive=value))

    def __ge__(prop, value):
        return prop.some(ConstrainedDatatype(type(value), min_inclusive=value))
    ...
    def _get_values_for_individual(Prop, entity):
        return IndividualValueList((entity.namespace.ontology._to_python(o, d) for o, d in
                                    entity.namespace.world._get_triples_sp_od(entity.storid, Prop.storid)),
                                   entity, Prop)
    ...
    def _set_value_for_individual(Prop, entity, value):
        if value is None:
            entity.namespace.ontology._del_triple_spod(entity.storid, Prop.storid, None, None)
        else:
            entity.namespace.ontology._set_triple_spod(entity.storid, Prop.storid,
                                                       *entity.namespace.ontology._to_rdf(value))
        ...
        if isinstance(entity, EntityClass):
            ...
            if Prop is entity.namespace.world._props.get(Prop._python_name):  # use cached value
                if Prop.is_functional_for(entity.__class__):
                    return FunctionalIndividualValueList([getattr(entity, Prop._python_name)], entity, Prop)
                else:
                    return getattr(entity, Prop._python_name)
            else:
...
class ObjectPropertyClass(ReasoningPropertyClass):
    ...
    def __init__(Prop, name, bases, obj_dict):
        ...
        if SymmetricProperty in Prop.is_a:
            ...
            if inverse_storid:
                type.__setattr__(Prop, "_inverse_property", Prop.namespace.world._get_by_storid(inverse_storid))
            else:
                type.__setattr__(Prop, "_inverse_property", None)
    ...
    def _class_is_a_changed(Prop, old):
        ...
        if (SymmetricProperty in old) and (not SymmetricProperty in Prop.is_a):
            if Prop._inverse_property:
                type.__setattr__(Prop, "_inverse_storid", Prop._inverse_property.storid)
            else:
                type.__setattr__(Prop, "_inverse_storid", 0)
    ...
    def _get_value_for_class(Prop, entity):
        if Prop._class_property_relation:
            return Prop._get_value_for_individual(entity)

        elif Prop._class_property_some:
            for r in _property_value_restrictions(entity, Prop):
                if (r.type == VALUE) or (r.type == SOME) or ((r.type == EXACTLY) and r.cardinality >= 1) or (
                        (r.type == MIN) and r.cardinality >= 1): return r.value
    ...
    def _get_values_for_individual(Prop, entity):
        if Prop._inverse_storid:
            return IndividualValueList((entity.namespace.ontology._to_python(o)
                                        for o in
                                        entity.namespace.world._get_obj_triples_spi_o(entity.storid, Prop.storid,
                                                                                      Prop._inverse_storid)),
                                       entity, Prop)
        else:
            return IndividualValueList((entity.namespace.ontology._to_python(o)
                                        for o in
                                        entity.namespace.world._get_obj_triples_sp_o(entity.storid, Prop.storid)),
                                       entity, Prop)
    ...
    def _get_inverse_values_for_individual(Prop, entity):
        if Prop._inverse_storid:
            return InverseIndividualValueList((entity.namespace.ontology._to_python(s)
                                               for s in entity.namespace.world._get_obj_triples_pio_s(Prop.storid,
                                                                                                      Prop._inverse_storid,
                                                                                                      entity.storid)),
                                              entity, Prop)
        else:
            return InverseIndividualValueList((entity.namespace.ontology._to_python(s)
                                               for s in entity.namespace.world._get_obj_triples_po_s(Prop.storid,
                                                                                                     entity.storid)),
                                              entity, Prop)
    ...
    def _get_values_for_class(Prop, entity):
        if Prop._class_property_relation:
            ...
            return ClassValueList(set(r.value for r in _property_value_restrictions(entity, Prop)
                                      if (r.type == VALUE) or (r.type == SOME) or (
    ...
    def _get_indirect_values_for_individual(Prop, entity):
        ...
        for P in Props:
            if issubclass(P, TransitiveProperty):
                if P._inverse_storid:
                    prop_storids.append((P.storid, P._inverse_storid))
                else:
                    prop_storids.append((P.storid, None))
            else:
                if P._inverse_storid:
                    values.update(onto._to_python(o)
                                  for eq in eqs
                                  for g in (world._get_obj_triples_sp_o(eq.storid, P.storid),
                                            world._get_obj_triples_po_s(P._inverse_storid, eq.storid))
                                  for o in g)
                else:
                    values.update(onto._to_python(o)
                                  for eq in eqs
        ...
        if prop_storids:
            for eq in eqs:
                new_values = [onto._to_python(o) for o in
                              world._get_obj_triples_transitive_sp_indirect(eq.storid, prop_storids)]
            ...
            if issubclass_python(Prop, TransitiveProperty):
                ...
                def walk(o):
                    ...
                    for r in _inherited_properties_value_restrictions(o, Props, set()):
                        ...
                        elif (r.type == SOME) or ((r.type == EXACTLY) and r.cardinality >= 1) or (
                                (r.type == MIN) and r.cardinality >= 1):
                ...
                values = set(r.value for r in _inherited_properties_value_restrictions(entity, Props, set())
                             if
                             (r.type == VALUE) or (r.type == SOME) or ((r.type == EXACTLY) and r.cardinality >= 1) or (
                                         (r.type == MIN) and r.cardinality >= 1))
    ...
    def _set_value_for_individual(Prop, entity, value):
        if value is None:
            entity.namespace.ontology._del_obj_triple_spo(entity.storid, Prop.storid, None)
        else:
            entity.namespace.ontology._set_obj_triple_spo(entity.storid, Prop.storid, value.storid)
        if (not instance(entity, EntityClass)) and (Prop is entity.namespace.world._props.get(Prop._python_name)):
            entity.__dict__[Prop.python_name] = value

    def _set_value_for_class(Prop, entity, value):
        Prop._get_values_for_class(entity).reinit([value])
...
class DataPropertyClass(ReasoningPropertyClass):
    ...
    def _get_value_for_class(Prop, entity):
        if Prop._class_property_relation:
            Prop._get_value_for_individual(entity)
    ...
    def _get_values_for_individual(Prop, entity):
        return IndividualValueList((entity.namespace.ontology._to_python(o, d)
                                    for o, d in
                                    entity.namespace.world._get_data_triples_sp_od(entity.storid, Prop.storid)),
                                   entity, Prop)
    ...
    def _set_value_for_individual(Prop, entity, value):
        if value is None:
            entity.namespace.ontology._del_data_triple_spod(entity.storid, Prop.storid, None, None)
        else:
            entity.namespace.ontology._set_data_triple_spod(entity.storid, Prop.storid,
                                                            *entity.namespace.ontology._to_rdf(value))
        if (not instance(entity, EntityClass)) and (Prop is entity.namespace.world._props.get(Prop._python_name)):
            entity.__dict__[Prop.python_name] = value
...
_TYPE_PROPS = {FunctionalProperty, InverseFunctionalProperty, TransitiveProperty, SymmetricProperty, AsymmetricProperty,
    ...
    def _set_ontology(self, ontology):
        if not LOADING:
            if self.ontology and not ontology:
                ...
                raise OwlReadySharedBlankNodeError(
                    "A PropertyChain cannot be shared by two ontologies. Please create a dupplicate.")
...
def destroy_entity(e, undoable=False):
    if undoable:
        undoer_objs = []; undoer_datas = []; undoer_bnodes = []; undoer_relations = []
    else:
        undoer_objs = undoer_datas = None; undoer_bnodes = None; undoer_relations = None

    if hasattr(e, "__destroy__"):
        e.__destroy__(undoer_objs, undoer_datas)
        ...
        if o:
            for r in relations:
                if (r == rdf_type) or (r == rdfs_subpropertyof) or (r == rdfs_subclassof):
                    ...
                    parents = [e.namespace.world._to_python(i) for i in
                               e.namespace.world._get_obj_triples_sp_o(storid, r)]
...
def _flatten_only(r):
    if isinstance(r.value, Or):
        for i in r.value.Classes:
            if isinstance(i, OneOf):
                yield from i.instances
            else:
                yield i
    else:
        if isinstance(r.value, OneOf):
            yield from r.value.instances
        else:
            yield r.value
    ...
    def _callback(self, obj, old):
        ...
        if self._Prop._owl_type == owl_object_property:
            ...
            for removed in old - new:
                ...
                if inverse:
                    obj.namespace.ontology._del_obj_triple_spo(removed.storid, inverse.storid,
                                                               obj.storid)  # Also remove inverse
            ...
            for removed in old - new:
                obj.namespace.ontology._del_data_triple_spod(obj.storid, self._Prop.storid,
                                                             obj.namespace.ontology._to_rdf(removed)[0], None)

            for added in new - old:
                obj.namespace.ontology._add_data_triple_spod(obj.storid, self._Prop.storid,
                                                             *obj.namespace.ontology._to_rdf(added))
...
class FunctionalIndividualValueList(IndividualValueList):
    ...
    def _callback(self, obj, old):
        ...
        if not isinstance(obj, EntityClass):  # Update cache
            if self:
                obj.__dict__[self._Prop.python_name] = self[0]
            else:
                obj.__dict__[self._Prop.python_name] = None
