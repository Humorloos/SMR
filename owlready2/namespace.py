...
from owlready2.base import _universal_abbrev_2_iri, _universal_iri_2_abbrev, \
    _universal_abbrev_2_datatype, _universal_datatype_2_abbrev
...
class Namespace(object):
    ...
    def __enter__(self):
        if self.ontology is None: raise ValueError(
            "Cannot assert facts in this namespace: it is not linked to an ontology! (it is probably a global namespace created by get_namespace(); please use your_ontology.get_namespace() instead)")
        if self.world.graph: self.world.graph.acquire_write_lock()
        # CURRENT_NAMESPACES.append(self)
        l = CURRENT_NAMESPACES.get()
        if l is None:
            CURRENT_NAMESPACES.set([self])
        else:
            l.append(self)
        return self
    ...
    def __repr__(self):
        return """%s.get_namespace("%s")""" % (self.ontology, self.base_iri)

    def __getattr__(self, attr):
        return self.world["%s%s" % (self.base_iri, attr)]  # return self[attr]

    def __getitem__(self, name):
        return self.world["%s%s" % (self.base_iri, name)]
...
class _GraphManager(object):
    ...
    def _get_obj_triples_transitive_sp(self, subject, predicate, already=None):
        return set()

    def _get_obj_triples_transitive_po(self, predicate, object, already=None):
        return set()

    def _get_obj_triples_transitive_sym(self, subject, predicate):
        return set()

    def _get_obj_triples_transitive_sp_indirect(self, subject,
                                                predicates_inverses,
                                                already=None):
        return set()

    def _get_obj_triples_spo_spo(self, subject=None, predicate=None,
                                 object=None):
        return []

    _get_triples_s_p = _get_obj_triples_spo_spo

    def _has_data_triple_spod(self, subject=None, predicate=None, object=None,
                              d=None):
        return False

    _has_obj_triple_spo = _has_data_triple_spod

    def _get_obj_triples_cspo(self, subject=None, predicate=None, object=None,
                              ontology_graph=None):
        return []
  ...
  AND q4.s=q1.s AND q4.p=? AND q4.o=?""",
    ...
    def _del_obj_triple_spo(self, s=None, p=None, o=None):
        ...
        if _LOG_LEVEL > 1:
            ...
            if o and not ((isinstance(o, int) and (o < 0)) or (
                    isinstance(o, str) and o.startswith(
                '"'))): o = self._unabbreviate(o)
    ...
    def _to_python(self, o, d=None, main_type=None, main_onto=None,
                   default_to_none=False):
        if d is None:
            if o < 0: return self._parse_bnode(o)
            if o in _universal_abbrev_2_datatype:
                return _universal_abbrev_2_datatype[o]
            else:
                return self.world._get_by_storid(o, None, main_type, main_onto,
                                                 None, default_to_none)
        else:
            return from_literal(o, d)
        raise ValueError
    ...
    def inconsistent_classes(self):
        for s in self._get_obj_triples_transitive_sym(owl_nothing,
                                                      owl_equivalentclass):
            if not s < 0: yield self.world._get_by_storid(s)
    ...
    def properties(self):
        return itertools.chain(self.data_properties(), self.object_properties(),
    ...
    def disjoint_classes(self):
        ...
        for c, s, p, o in self._get_obj_triples_cspo_cspo(None, None,
                                                          owl_disjointwith,
                                                          None):
            with LOADING: a = AllDisjoint((s, p, o),
                                          self.world.graph.context_2_user_context(
                                              c), None)
            yield a  # Must yield outside the with statement

    def disjoint_properties(self):
        for s in self._get_obj_triples_po_s(rdf_type,
    ...
    def disjoints(self):
        return itertools.chain(self.disjoint_classes(),
                               self.disjoint_properties(),
                               self.different_individuals())

    def general_axioms(self):
        for s in itertools.chain(
                self._get_obj_triples_po_s(rdf_type, owl_restriction),
    ...
    def search(self, _use_str_as_loc_str=True, _case_sensitive=True, **kargs):
        ...
        for k, v0 in kargs.items():
            if isinstance(v0, _SearchMixin) or not isinstance(v0, list): v0 = (
                v0,)
            for v in v0:
                if k == "iri":
                    prop_vals.append((" iri", v, None))
                elif (k == "is_a") or (k == "subclass_of") or (k == "type"):
                    if isinstance(v, (_SearchMixin, Or)):
                        v2 = v
                    ...
                    if v is None:
                        ...
                        if isinstance(v, FTS):
                            ...
                            v2 = v;
                            d = "*"
                        elif isinstance(v, _SearchMixin):
                            v2 = v
                        else:
                            v2, d = self.world._to_rdf(v)
                            if Prop and (
                                    Prop._owl_type == owl_object_property):  # Use "*" as a jocker for object
                                d = None
                            elif Prop and (
                                    Prop._owl_type == owl_annotation_property) and (
                                    v2 == "*"):  # Use "*" as a jocker for annotation
                                d = "quads"
                            elif ((not d is None) and (
                                    isinstance(v2, (int, float)))) or (
                                    _use_str_as_loc_str and (
                                    d == 60)):  # A string, which can be associated to a language in RDF
                                d = "*"
    ...
    def search_one(self, **kargs):
        return self.search(**kargs).first()
...
class World(_GraphManager):
    def __init__(self, backend="sqlite", filename=":memory:",
                 dbname="owlready2_quadstore", **kargs):
        global owl_world
        ...
        if not owl_world is None:
            self._entities.update(
                owl_world._entities)  # add OWL entities in the world
    ...
    def set_backend(self, backend="sqlite", filename=":memory:",
        ...
        if backend == "sqlite":
            ...
            if self.graph and len(self.graph):
                self.graph = Graph(filename, world=self, clone=self.graph,
                                   **kargs)
            else:
        ...
        for iri in self.graph.ontologies_iris():
            self.get_ontology(
                iri)  # Create all possible ontologies if not yet done

        self._full_text_search_properties = CallbackList(
            [self._get_by_storid(storid, default_to_none=True) or storid for
             storid in self.graph.get_fts_prop_storid()], self,
            World._full_text_search_changed)

    def close(self):
        self.graph.close()

    def get_full_text_search_properties(self):
        return self._full_text_search_properties

    def set_full_text_search_properties(self, l):
        old = self._full_text_search_properties
        self._full_text_search_properties = CallbackList(l, self,
                                                         World._full_text_search_changed)
        self._full_text_search_changed(old)

    full_text_search_properties = property(get_full_text_search_properties,
                                           set_full_text_search_properties)
    ...
    def new_blank_node(self):
        return self.graph.new_blank_node()
    ...
    def save(self, file=None, format="rdfxml", **kargs):
        if file is None:
            ...
            if _LOG_LEVEL: print(
                "* Owlready2 * Saving world %s to %s..." % (self, file),
                file=sys.stderr)
            file = open(file, "wb")
            self.graph.save(file, format, **kargs)
            file.close()
        else:
            if _LOG_LEVEL: print("* Owlready2 * Saving world %s to %s..." % (
                self, getattr(file, "name", "???")), file=sys.stderr)
            self.graph.save(file, format, **kargs)

    def as_rdflib_graph(self):
        if self._rdflib_store is None:
            import owlready2.rdflib_store
            self._rdflib_store = owlready2.rdflib_store.TripleLiteRDFlibStore(
                self)
    ...
    def get_ontology(self, base_iri):
        if (not base_iri.endswith("/")) and (not base_iri.endswith("#")):
            if ("%s#" % base_iri) in self.ontologies:
                base_iri = base_iri = "%s#" % base_iri
            elif ("%s/" % base_iri) in self.ontologies:
                base_iri = base_iri = "%s/" % base_iri
            else:
                base_iri = base_iri = "%s#" % base_iri
    ...
    def _get_by_storid(self, storid, full_iri=None, main_type=None,
                       main_onto=None, trace=None, default_to_none=True):
        entity = self._entities.get(storid)
        if not entity is None: return entity

        try:
            return self._load_by_storid(storid, full_iri, main_type, main_onto,
                                        default_to_none)
        except RecursionError:
            return self._load_by_storid(storid, full_iri, main_type, main_onto,
                                        default_to_none, ())

    def _load_by_storid(self, storid, full_iri=None, main_type=None,
                        main_onto=None, default_to_none=True, trace=None):
        with LOADING:
            types = []
            is_a_bnodes = []
            for graph, obj in self._get_obj_triples_sp_co(storid, rdf_type):
                if main_onto is None: main_onto = self.graph.context_2_user_context(
                    graph)
                if obj == owl_class:
                    main_type = ThingClass
                elif obj == owl_object_property:
                    main_type = ObjectPropertyClass;
                    types.append(
                        ObjectProperty)
                elif obj == owl_data_property:
                    main_type = DataPropertyClass;
                    types.append(
                        DataProperty)
                elif obj == owl_annotation_property:
                    main_type = AnnotationPropertyClass;
                    types.append(
                        AnnotationProperty)
                    ...
                    if obj < 0:
                        is_a_bnodes.append(
                            (self.graph.context_2_user_context(graph), obj))
                    else:
                        Class = self._get_by_storid(obj, None, ThingClass,
                                                    main_onto)
                        if isinstance(Class, EntityClass):
                            types.append(Class)
                        elif Class is None:
                            raise ValueError("Cannot get '%s'!" % obj)

            if main_type is None:  # Try to guess it
                if self._has_obj_triple_spo(None, rdf_type,
                                            storid) or self._has_obj_triple_spo(
                    None, rdfs_subclassof, storid) or self._has_obj_triple_spo(
                    storid, rdfs_subclassof, None):
                    main_type = ThingClass
                elif self._has_obj_triple_spo(storid, None,
                                              None) or self._has_data_triple_spod(
                    storid, None, None, None):
                    main_type = Thing
            ...
            if main_type and (not main_type is Thing):
                if not trace is None:
                    if storid in trace:
                        s = "\n  ".join(
                            [(i if i < 0 else self._unabbreviate(i)) for i in
                             trace[trace.index(storid):]])
                        print(
                ...
                is_a_entities = []
                for graph, obj in self._get_obj_triples_sp_co(storid,
                                                              main_type._rdfs_is_a):
                    if obj < 0:
                        is_a_bnodes.append(
                            (self.graph.context_2_user_context(graph), obj))
                    else:
                        obj2 = self._entities.get(obj)
                        if obj2 is None: obj2 = self._load_by_storid(obj, None,
                                                                     main_type,
                                                                     main_onto,
                                                                     default_to_none,
                                                                     trace)
            ...
            if main_onto is None:
                ...
                if full_iri.startswith(owl.base_iri) or full_iri.startswith(
                        rdfs.base_iri) or full_iri.startswith(
                    "http://www.w3.org/1999/02/22-rdf-syntax-ns#"): return None
            ...
            if main_type is ThingClass:
                ...
                entity = ThingClass(name, types,
                                    {"namespace": namespace, "storid": storid})

            elif main_type is ObjectPropertyClass:
                try:
                    types = tuple(t for t in types if
                                  t.iri != "http://www.w3.org/1999/02/22-rdf-syntax-ns#Property")
                    entity = ObjectPropertyClass(name,
                                                 types or (ObjectProperty,),
                                                 {"namespace": namespace,
                                                  "is_a": is_a_entities,
                                                  "storid": storid})
                    ...
                    if e.args[0].startswith("metaclass conflict"):
                        print(
                            "* Owlready2 * WARNING: ObjectProperty %s belongs to more than one entity types: %s; I'm trying to fix it..." % (
                                full_iri, list(types) + is_a_entities),
                            file=sys.stderr)
                        is_a_entities = [t for t in is_a_entities if
                                         issubclass_python(t, ObjectProperty)]
                        try:
                            entity = ObjectPropertyClass(name, types or (
                ...
                try:
                    types = tuple(t for t in types if
                                  t.iri != "http://www.w3.org/1999/02/22-rdf-syntax-ns#Property")
                    entity = DataPropertyClass(name, types or (DataProperty,),
                                               {"namespace": namespace,
                                                "is_a": is_a_entities,
                                                "storid": storid})
                except TypeError as e:
                    if e.args[0].startswith("metaclass conflict"):
                        print(
                            "* Owlready2 * WARNING: DataProperty %s belongs to more than one entity types: %s; I'm trying to fix it..." % (
                                full_iri, list(types) + is_a_entities),
                            file=sys.stderr)
                        is_a_entities = [t for t in is_a_entities if
                                         issubclass_python(t, DataProperty)]
                        # entity = DataPropertyClass(name, types or (DataProperty,), { "namespace" : namespace, "is_a" : is_a_entities, "storid" : storid } )
                        entity = DataPropertyClass(name, (DataProperty,),
                                                   {"namespace": namespace,
                                                    "is_a": is_a_entities,
                                                    "storid": storid})
                ...
                try:
                    types = tuple(t for t in types if
                                  t.iri != "http://www.w3.org/1999/02/22-rdf-syntax-ns#Property")
                    entity = AnnotationPropertyClass(name, types or (
                        AnnotationProperty,), {"namespace": namespace,
                                               "is_a": is_a_entities,
                                               "storid": storid})
                except TypeError as e:
                    if e.args[0].startswith("metaclass conflict"):
                        print(
                            "* Owlready2 * WARNING: AnnotationProperty %s belongs to more than one entity types: %s; I'm trying to fix it..." % (
                                full_iri, list(types) + is_a_entities),
                            file=sys.stderr)
                        is_a_entities = [t for t in is_a_entities if
                                         issubclass_python(t,
                                                           AnnotationProperty)]
                        entity = AnnotationPropertyClass(name,
                                                         (AnnotationProperty,),
                                                         {
                                                             "namespace": namespace,
                                                             "is_a": is_a_entities,
                ...
                if len(types) == 1:
                    ...
                    Class = FusionClass._get_fusion_class(types)
                else:
                    Class = Thing
            ...
            if is_a_bnodes:
                list.extend(entity.is_a,
                            (onto._parse_bnode(bnode) for onto, bnode in
                             is_a_bnodes))
...
class Ontology(Namespace, _GraphManager):
    def __init__(self, world, base_iri, name=None):
        ...
        self._imported_ontologies = CallbackList([], self,
                                                 Ontology._import_changed)
        ...
        if _LOG_LEVEL: print("* Owlready2 * Creating new ontology %s <%s>." % (
            self.name, self.base_iri), file=sys.stderr)

        if (not LOADING) and (not self.graph is None):
            if not self._has_obj_triple_spo(self.storid, rdf_type,
                                            owl_ontology):
                if self.world.graph: self.world.graph.acquire_write_lock()
    ...
    def destroy(self):
        ...
        for entity in list(self.world._entities.values()):
            if entity.namespace.ontology is self: del self.world._entities[
                entity.storid]
    ...
    def get_imported_ontologies(self):
        return self._imported_ontologies

    def set_imported_ontologies(self, l):
        old = self._imported_ontologies
        self._imported_ontologies = CallbackList(l, self,
                                                 Ontology._import_changed)
        self._import_changed(old)

    imported_ontologies = property(get_imported_ontologies,
    ...
    def set_python_module(self, module_name):
        self._set_data_triple_spod(self.storid, owlready_python_module,
                                   module_name, 0)
    ...
    def get_namespace(self, base_iri, name=""):
        if (not base_iri.endswith("/")) and (
                not base_iri.endswith("#")): base_iri = "%s#" % base_iri
    ...
    def load(self, only_local=False, fileobj=None, reload=False,
             reload_if_newer=False, **args):
        ...
        if self.base_iri == "http://www.lesfleursdunormal.fr/static/_downloads/owlready_ontology.owl#":
            ...
            f = fileobj or _get_onto_file(self.base_iri, self.name, "r",
                                          only_local)
        else:
            f = ""
        ...
        if f.startswith("http:") or f.startswith("https:"):
            if reload or (
                    self.graph.get_last_update_time() == 0.0):  # Never loaded
                if _LOG_LEVEL: print(
                    "* Owlready2 *     ...loading ontology %s from %s..." % (
                        self.name, f), file=sys.stderr)
                try:
                    ...
                    raise OwlReadyOntologyParsingError(
                        "Cannot download '%s'!" % f)
                try:
                    new_base_iri = self.graph.parse(fileobj,
                                                    default_base=self.base_iri,
                                                    **args)
                finally:
                    fileobj.close()
        elif fileobj:
            if _LOG_LEVEL: print(
                "* Owlready2 *     ...loading ontology %s from %s..." % (
                    self.name,
                    getattr(fileobj, "name", "") or getattr(fileobj, "url",
                                                            "???")),
                file=sys.stderr)
            try:
                new_base_iri = self.graph.parse(fileobj,
            ...
            if reload or (reload_if_newer and (os.path.getmtime(f) > self.graph.get_last_update_time())) or (
                ...
                if _LOG_LEVEL: print(
                    "* Owlready2 *     ...loading ontology %s (cached)..." % self.name,
                    file=sys.stderr)
        ...
        if new_base_iri and (new_base_iri != self.base_iri):
            ...
            self._namespaces[self.base_iri] = self.world.ontologies[
                self.base_iri] = self
            ...
            self.metadata = Metadata(self,
                                     self.storid)  # Metadata depends on storid

        elif not self.graph._has_obj_triple_spo(self.storid, rdf_type,
                                                owl_ontology):  # Not always present (e.g. not in dbpedia)
            if self.world.graph: self.world.graph.acquire_write_lock()
        ...
        imported_ontologies = [
            self.world.get_ontology(self._unabbreviate(abbrev_iri)).load() for
            abbrev_iri in
            self.world._get_obj_triples_sp_o(self.storid, owl_imports)]
        self._imported_ontologies._set(imported_ontologies)

        # Import Python module
        for module, d in self._get_data_triples_sp_od(self.storid,
                                                      owlready_python_module):
            module = from_literal(module, d)
            if _LOG_LEVEL: print(
                "* Owlready2 *     ...importing Python module %s required by ontology %s..." % (
                    module, self.name), file=sys.stderr)
    ...
    def _load_properties(self):
        ...
        for prop_storid in itertools.chain(
                self._get_obj_triples_po_s(rdf_type, owl_object_property),
            ...
            python_name_d = self.world._get_data_triple_sp_od(prop_storid,
            ...
            if python_name_d is None:
                ...
                with LOADING:
                    Prop.python_name = python_name_d[0]
                props.append("%s (%s)" % (Prop.python_name, Prop.name))
        if _LOG_LEVEL:
            print("* Owlready2 *     ...%s properties found: %s" % (
                len(props), ", ".join(props)), file=sys.stderr)
    ...
    def indirectly_imported_ontologies(self, already=None):
        ...
        if not self in already:
            already.add(self)
            yield self
            for ontology in self._imported_ontologies: yield from ontology.indirectly_imported_ontologies(
                already)

    def save(self, file=None, format="rdfxml", **kargs):
        if file is None:
            file = _open_onto_file(self.base_iri, self.name, "wb")
            if _LOG_LEVEL: print("* Owlready2 * Saving ontology %s to %s..." % (
                self.name, getattr(file, "name", "???")), file=sys.stderr)
            self.graph.save(file, format, **kargs)
            file.close()
        elif isinstance(file, str):
            if _LOG_LEVEL: print(
                "* Owlready2 * Saving ontology %s to %s..." % (self.name, file),
                file=sys.stderr)
            file = open(file, "wb")
            self.graph.save(file, format, **kargs)
            file.close()
    ...
    def _add_data_triple_spod(self, s, p, o, d):
        ...
        if _LOG_LEVEL > 1:
            ...
            if isinstance(d, str) and (
                    not d.startswith("@")): d = self._unabbreviate(d)
    ...
    def _add_annotation_axiom(self, source, property, target, target_d, annot,
                              value, d):
        for bnode in self.world._get_annotation_axioms(source, property, target,
                                                       target_d):
            break  # Take first
        else:
            ...
            if target_d is None:
                ...
                self._add_data_triple_spod(bnode, owl_annotatedtarget, target,
                                           target_d)

        if d is None:
            self._add_obj_triple_spo(bnode, annot, value)
        else:
            self._add_data_triple_spod(bnode, annot, value, d)
        return bnode

    def _del_annotation_axiom(self, source, property, target, target_d, annot,
                              value, d):
        for bnode in self._get_obj_triples_po_s(rdf_type, owl_axiom):
            ok = False
            other = False
            for p, o, d in self._get_triples_s_pod(bnode):
                ...
                if ok:
                    if other:
                        if d is None:
                            self._del_obj_triple_spo(bnode, annot, value)
                        else:
                            self._del_data_triple_spod(bnode, annot, value,
                                                       None)
                    else:
    ...
    def _reload_bnode(self, bnode):
        if bnode in self._bnodes:
            ...
            equivalentclasses = {entity for entity in subclasses if
                                 old in entity.equivalent_to}
    ...
    def _parse_bnode(self, bnode):
        ...
        with LOADING:
            ...
            for pred, obj in self._get_obj_triples_s_po(bnode):
                if pred == owl_complementof:
                    r = Not(None, self,
                            bnode);
                    break  # will parse the rest on demand
                elif pred == owl_unionof:
                    r = Or(obj, self, bnode);
                    break
                elif pred == owl_intersectionof:
                    r = And(obj, self, bnode);
                    break
                # elif pred == owl_disjointunion:  r = DisjointUnion(obj , self, bnode); break

                elif pred == owl_onproperty:
                    restriction_property = self._to_python(obj, None)
                    ...
                    restriction_type = HAS_SELF;

                elif pred == owl_oneof:
                    r = OneOf(self._parse_list(obj), self, bnode);
                    break

                elif pred == owl_members:
                    members = obj
                elif pred == owl_distinctmembers:
                    members = obj
                    ...
                    if obj == owl_alldisjointclasses:
                        Disjoint = AllDisjoint
                    elif obj == owl_alldisjointproperties:
                        Disjoint = AllDisjoint
                    elif obj == owl_alldifferent:
                        Disjoint = AllDisjoint

                    elif obj == owl_axiom:
                        return None

                elif pred == owl_ondatatype:
                    on_datatype = _universal_abbrev_2_datatype[obj]
                elif pred == owl_withrestrictions:
                    with_restriction = obj

            else:
                if restriction_type:
                    r = Restriction(restriction_property, restriction_type,
                                    None, None, self, bnode)
                elif Disjoint:
                    r = Disjoint(members, self, bnode)
                elif on_datatype and with_restriction:
                    r = ConstrainedDatatype(on_datatype, self, bnode,
                                            with_restriction)
                else:
                    for pred, obj, d in self._get_data_triples_s_pod(bnode):
                        if pred == VALUE:
                            ...
                            restriction_cardinality = self._to_python(
                                obj, d)
                        elif pred == MIN:
                            ...
                            restriction_cardinality = self._to_python(
                                obj, d)
                        elif pred == owl_cardinality:
                            restriction_type = EXACTLY;
                            restriction_cardinality = self._to_python(
                                obj, d)
                        elif pred == owl_min_cardinality:
                            restriction_type = MIN;
                            restriction_cardinality = self._to_python(
                                obj, d)
                        elif pred == owl_max_cardinality:
                            restriction_type = MAX;
                            restriction_cardinality = self._to_python(
                                obj, d)
                        if restriction_type:
                            r = Restriction(restriction_property,
                                            restriction_type,
                                            restriction_cardinality, None, self,
                                            bnode)
                            break
                        # else:
                        #  s = ""
    ...
    def _set_list(self, bnode, l):
        ...
        for i in range(len(l)):
            ...
            if d is None:
                self._add_obj_triple_spo(bnode, rdf_first, o)
            else:
                self._add_data_triple_spod(bnode, rdf_first, o, d)
            if i < len(l) - 1:
    ...
    def _set_list_as_rdf(self, bnode, l):
        ...
        for i in range(len(l)):
            if l[i][1] is None:
                self._add_obj_triple_spo(bnode, rdf_first, l[i][0])
            else:
                self._add_data_triple_spod(bnode, rdf_first, l[i][0], l[i][1])
            if i < len(l) - 1:
                bnode_next = self.world.new_blank_node()
                self._add_obj_triple_spo(bnode, rdf_rest, bnode_next)
                bnode = bnode_next
    ...
    def __repr__(self):
        return """get_ontology("%s")""" % (self.base_iri)
...
class Metadata(object):
    ...
    def __getattr__(self, attr):
        ...
        values = [self.namespace.ontology._to_python(o, d) for o, d in
                  self.namespace.world._get_triples_sp_od(self.storid,
                                                          Prop.storid)]
    ...
    def __setattr__(self, attr, values):
        ...
        if isinstance(Prop, AnnotationPropertyClass):
            ...
            raise ValueError(
                "Metadata can only use defined annotation properties!")


def _open_onto_file(base_iri, name, mode="r", only_local=False):
    if base_iri.endswith("#") or base_iri.endswith("/"): base_iri = base_iri[
                                                                    :-1]
    ...
    if (mode.startswith("r")) and not only_local: return urllib.request.urlopen(
        base_iri)
    if (mode.startswith("w")): return open(
        os.path.join(onto_path[0], "%s.owl" % name), mode)
...
def _get_onto_file(base_iri, name, mode="r", only_local=False):
    ...
    if (mode.startswith("w")): return os.path.join(onto_path[0],
                                                   "%s.owl" % name)
    raise FileNotFoundError
